...
export const AppLayout: React.FC = () => {
  ...
  const errorMessage = useTranscriptionStore((s) => s.errorMessage);

  // "No Model" dialog state
  const navigate = useNavigate();
  const [showNoModelDialog, setShowNoModelDialog] = React.useState(false);

  React.useEffect(() => {
    if (errorMessage === 'NO_MODELS_DOWNLOADED') {
      setShowNoModelDialog(true);
    }
  }, [errorMessage]);

  const handleCloseNoModelDialog = React.useCallback(() => {
    setShowNoModelDialog(false);
    useTranscriptionStore.setState({ errorMessage: undefined });
  }, []);

  const handleGoToModelDownload = React.useCallback(() => {
    setShowNoModelDialog(false);
    useTranscriptionStore.setState({ errorMessage: undefined });
  ...
  return (
    <FluentProvider theme={theme}>
      ...
      <UpdateNotification />
      <Dialog
        open={showNoModelDialog}
        onOpenChange={(_, data) => {
          if (!data.open) handleCloseNoModelDialog();
        }}
      >
        <DialogSurface>
          <DialogBody>
            <DialogTitle>No Transcription Model Available</DialogTitle>
            <DialogContent>
              A transcription model is required to start recording. Please download a model to
              continue.
            </DialogContent>
            <DialogActions>
              <Button appearance="secondary" onClick={handleCloseNoModelDialog}>
                Cancel
              </Button>
              <Button appearance="primary" onClick={handleGoToModelDownload}>
                Download Model
              </Button>
            </DialogActions>
          </DialogBody>
