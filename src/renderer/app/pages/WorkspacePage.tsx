...
export default function WorkspacePage() {
  ...
  // Refresh current note when sync completes to get latest content
  useSyncRefresh(
    React.useCallback(async () => {
      // Only refresh if we have a specific note loaded
      if (!noteId) {
        return;
      }
      // Avoid clobbering in-flight edits; let the local editor finish saving first
      ...
      try {
        ...
        setNote(data);
        // Only update title if user hasn't modified it in the editor
        if (!currentEditorTitle || currentEditorTitle === note?.meta.title) {
          setCurrentEditorTitle(data.meta.title || '');
        }
      } catch (error) {
        console.error('Failed to refresh note after sync:', noteId, error);
      }
    }, [noteId, currentEditorTitle, note?.meta.title])
  );
  ...
  const editor = !noteId ? (
    ...
    <Editor
      key={`note-${note.meta.id}-${note.meta.updatedAt.getTime()}`}
      binderId={editorBinderId}
      noteId={note.meta.id}
