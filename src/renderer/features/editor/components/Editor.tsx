  ...
  const createIfNeeded = React.useCallback(
    async (hasContent: boolean) => {
      ...
      try {
        ...
        // When onNoteIdChange is provided, the parent (WorkspacePage) handles
        // navigation and event dispatch so the Editor key stays stable.
        if (onNoteIdChange) {
          onNoteIdChange(newId);
        } else {
          // Fallback: handle navigation directly when no parent callback exists
          window.dispatchEvent(new Event('notes:changed'));
          navigate(`/binders/${binderId}/notes/${newId}`, { replace: true });
        }
        return newId;
      } catch (error) {
    ...
    [binderId, navigate, onNoteIdChange]
  );
