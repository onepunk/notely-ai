...
export function SetupScreen({ onReady }: SetupScreenProps): JSX.Element {
  ...
  const statusMessage = React.useMemo(() => {
    switch (phase) {
      ...
      case 'downloading':
        ...
        if (currentDownload === 'llm-server') return 'Downloading AI engine...';
  ...
  React.useEffect(() => {
    ...
    const unsubProgress = window.api.components.onDownloadProgress((progress: DownloadProgress) => {
      ...
      setOverallProgress(Math.min(100, progress.overallPercent));
  ...
  return (
    <div className={styles.container}>
      ...
      <h1 className={styles.title}>Notely</h1>
