...
export const TranscriptionSidebar: React.FC<Props> = ({
  ...
  const _canGenerateSummary = useCanGenerateSummary();
  ...
  const handleSummaryClick = React.useCallback(async () => {
    ...
    // License check is already done via showSummaryButton (hasActiveLicense)
    // No authentication required for Notely AI standalone - everything is local
  ...
  }, [isSummarizing, isRecording, transcriptionId, setActiveTab]);
  ...
  const handleModelLoaded = React.useCallback(() => {
    ...
    // Use setTimeout to allow state to settle after modal close
    setTimeout(() => {
      handleSummaryClick();
    }, 0);
  }, [handleSummaryClick]);
