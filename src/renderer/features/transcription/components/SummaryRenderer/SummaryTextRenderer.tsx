...
export function parseSummaryText(text: string): ParsedSummary {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check for markdown headings: # Title, ## Section, ### Subsection
    ...
    if (mdHeadingMatch) {
      ...
      const headerText = mdHeadingMatch[2].trim();

      // # at document start (or level-1 heading) becomes the title
      if (level === 1 && !result.title) {
        result.title = headerText;
        continue;
      }

      // ## and ### become section headers
      currentSection = {
