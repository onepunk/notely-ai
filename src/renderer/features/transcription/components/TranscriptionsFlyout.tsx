...
import { useSyncRefresh } from '../../../shared/hooks/useSyncEvents';
...
export const TranscriptionsFlyout: React.FC<Props> = ({
    ...
    try {
      ...
      const listWithContent = await Promise.all(
        list.map(async (item: Record<string, unknown>) => {
          try {
            const content = await window.api.transcription.getContent(item.id as string);
            return {
              id: item.id as string,
              language: item.language as string,
              status: item.status as string,
              startTime: new Date((item.start_time ?? item.startTime) as number),
              endTime:
                item.end_time || item.endTime
                  ? new Date((item.end_time ?? item.endTime) as number)
                  : undefined,
              durationMs: (item.duration_ms ?? item.durationMs) as number | undefined,
              charCount: (item.char_count ?? item.charCount) as number,
              wordCount: (item.word_count ?? item.wordCount) as number,
              updatedAt: new Date((item.updated_at ?? item.updatedAt) as number),
  ...
  // Listen for sync completion to refresh transcriptions if flyout is open
  useSyncRefresh(() => {
    if (isOpen) {
      loadTranscriptions();
    }
