...
import { LoadModelModal, useCanGenerateSummary, useLLMStore } from '@features/llm';
...
export const TranscriptionViewModal: React.FC<Props> = ({ transcription, isOpen, onClose }) => {
  ...
  const canGenerateSummary = useCanGenerateSummary();
  const [showSignInModal, setShowSignInModal] = React.useState(false);
  const [showLoadModelModal, setShowLoadModelModal] = React.useState(false);
  ...
  const [segments, setSegments] = React.useState<
    Array<{
      ...
      startTime: number;
      endTime: number;
      speaker?: 'user' | 'participant' | 'both' | 'unknown';
    }>
  >([]);
  ...
  const handleGenerateSummary = React.useCallback(async () => {
    ...
    // Check if model is loaded before proceeding — read directly from the store
    // to avoid stale React closure values after model loading
    const llmState = useLLMStore.getState();
    const canGenerate =
      llmState.modelLoaded && llmState.serverStatus === 'ready' && llmState.gpuDetected;
    if (!canGenerate) {
      setShowLoadModelModal(true);
      return;
    }
  ...
  }, [transcription.id, isAuthenticated, summaryJobStatus, summary]);

  // Callback when model is loaded from the modal - re-trigger summary generation
  const handleModelLoaded = React.useCallback(() => {
    setShowLoadModelModal(false);
    // Re-trigger summary generation after model loads
    setTimeout(() => {
  ...
  // Format content with timestamps from grouped segments
  const formattedContent = React.useMemo(() => {
    ...
    return groups
      .map((group) => {
        const mins = Math.floor(group.startTime / 60);
        const secs = Math.floor(group.startTime % 60);
        const timestamp = `[${mins}:${secs.toString().padStart(2, '0')}]`;
        const timeMs = (mins * 60 + secs) * 1000;

        const speakerLabel =
          group.speaker && group.speaker !== 'unknown'
            ? ` <span class="speaker-label">${group.speaker === 'user' ? 'You' : group.speaker === 'both' ? 'Both' : 'Participant'}</span>`
            : '';

        return (
          `<div class="paragraph-group">` +
          `<span class="timestamp" data-time-ms="${timeMs}">${timestamp}</span>${speakerLabel}<br/>` +
          `${group.text}` +
          `</div>`
        );
      })
  ...
  return (
    <>
      ...
      />
      <LoadModelModal
        isOpen={showLoadModelModal}
        onClose={() => setShowLoadModelModal(false)}
