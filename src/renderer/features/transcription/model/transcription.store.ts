 ...
 * TranscriptionConfig overrides object.  Reads from the new `transcription.*`
 * keys written by the Model Settings > Transcription UI, plus legacy
 * `transcription.advanced.*` keys for backward compatibility.
 *
 * Empty/unset values are omitted so that buildTranscriptionConfig() in
 * config.ts falls through to TRANSCRIPTION_CONFIG defaults.
 */
function readAdvancedConfigOverrides(): Partial<TranscriptionConfig> {
  ...
  //    These take precedence over transcription.* keys if both are set.
  ...
  return {
    ...
    async start(options) {
      ...
      try {
        ...
        set({
          ...
          detectedLanguage: undefined, // Reset detected language on new session
          languageConfidence: undefined,
        });
        ...
        if (httpPort) {
          try {
            const statusUrl = `http://127.0.0.1:${httpPort}/models/status`;
            console.log('Fetching model status from:', statusUrl);
            const modelsResp = await fetch(statusUrl);
            const modelsData = (await modelsResp.json()) as {
              models: Array<{ id: string; downloaded: boolean }>;
            };
            console.log(
              'Model status response:',
              JSON.stringify(
                modelsData.models?.map((m) => ({ id: m.id, downloaded: m.downloaded }))
              )
            );
            const anyDownloaded = modelsData.models.some((m) => m.downloaded);
            if (!anyDownloaded) {
              console.error('No models downloaded — aborting recording');
              set({
        ...
        const client = new TranscriptionClient({
          ...
          onClose: () => {
            ...
            // If we're recording and the WebSocket closes unexpectedly,
            // stop the recording so the user doesn't keep recording with no transcription
            if (get().isRecording) {
              console.warn('WebSocket closed unexpectedly during recording — stopping');
    ...
    setPartial(text) {
      ...
      if (typeof text === 'object' && 'stable' in text && 'unstable' in text) {
        ...
        //   void _persistStableText(stableText);
        // }
      } else {
        // Protocol v1 (string)
          ...
          if (hasUserEdits && originalDisplayText && displayText) {
            ...
            for (const { original, corrected } of corrections) {
              ...
              if (beforeReplace !== finalText) {
                console.log('Refine: Applied user correction');
              }
            }
          }
...
(window as any).__ZUSTAND_STORES__.transcription = useTranscriptionStore;
