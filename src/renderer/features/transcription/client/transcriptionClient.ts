...
export class TranscriptionClient {
    ...
    await new Promise<void>((resolve, reject) => {
      try {
        ...
        ws.onclose = (e) => {
          ...
          this.resolveFinalAwaiter();
          // If the WebSocket closed unexpectedly (server-side error, not user stop),
          // emit an error so the UI can notify the user
          if (!this.isStopping && e.code !== 1000) {
            console.error('TranscriptionClient: WebSocket closed unexpectedly during recording');
            this.options.onError?.(new Error(`Transcription server disconnected (code ${e.code})`));
          }
        };
      } catch (err) {
  ...
  private sendConfig(): void {
    ...
    try {
      ...
      // The dev Python server handles 'auto' itself, but the packaged audio-engine does not.
      const serverConfig = {
        ...config,
