...
export class CertificatePinningService extends EventEmitter {
  ...
  initialize(): void {
    ...
    logger.info('CertificatePinningService: Initialized and ENABLED');
    this.initialized = true;
  }
  ...
  private extractSPKIPinFromTLSCert(cert: tls.PeerCertificate): string {
    if (cert.raw) {
      try {
        return native.computeSpkiPin(cert.raw);
      } catch (error) {
        logger.error('CertificatePinningService: Failed to compute SPKI pin', {
          error: error instanceof Error ? error.message : String(error),
        });
        return '';
      }
    ...
    logger.warn('CertificatePinningService: Certificate missing raw data');
    return '';
  }
