...
import { getEncryptionKeyManager } from '../../storage/core/EncryptionKeyManager';
...
export class PasswordProtectionService {
   ...
   * Uses the native addon for compiled C++ PBKDF2 + AES-GCM.
   */
  private async encryptKeyWithPassword(key: string, password: string): Promise<EncryptedKeyBlob> {
    return native.encryptKeyWithPassword(key, password);
  }
  ...
  private async decryptKeyWithPassword(blob: EncryptedKeyBlob, password: string): Promise<string> {
    try {
      return await native.decryptKeyWithPassword(blob, password);
    } catch {
