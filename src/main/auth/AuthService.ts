...
export class AuthService extends EventEmitter implements IAuthService {
  ...
  async authenticateWithPassword(email: string, password: string): Promise<AuthResult> {
    ...
    try {
      ...
      const profile = await this.fetchUserProfile(serverUrl, data.token);
      ...
      await this.keystoreService.setAccessToken(data.token);
   ...
   * Uses sync.device_id as the single source of truth
   */
  private async getDeviceId(): Promise<string | null> {
    ...
    const storedId = await this.options.storage.settings.get('sync.device_id');
    if (storedId) {
    ...
    await this.options.storage.settings.set('sync.device_id', newId);
    this.deviceId = newId;
    return newId;
  }
