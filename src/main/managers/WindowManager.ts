...
export class WindowManager {
  ...
  async createMainWindow(): Promise<BrowserWindow | null> {
    logger.info('WindowManager: Starting window creation process');
    ...
    try {
      ...
      logger.info('WindowManager: Creating BrowserWindow instance');
      ...
      this.mainWindow = new BrowserWindow({
        ...
        icon: iconPath,
        webPreferences: {
          // Prevent Chromium from throttling audio rendering when the window
          // loses focus.  Critical for the transcription pipeline which relies
          // on a continuously-running AudioWorklet / ScriptProcessorNode.
      ...
      logger.info('WindowManager: BrowserWindow created successfully');
      ...
      logger.info('WindowManager: Window creation process completed successfully');
  ...
  private resolvePreloadScript(): string | null {
    ...
    logger.info('WindowManager: Preload script resolution', {
      __dirname,
      preloadJs,
      preloadCjs,
      chosen: preloadPath,
  ...
  private getIconPath(): string {
    ...
    logger.info('WindowManager: Icon path resolution', {
  ...
  private logInitialWindowBounds(): void {
    try {
      ...
      logger.info('WindowManager: Initial window size', { bounds, contentBounds });
    } catch (error) {
  ...
  private setupWindowEventListeners(): void {
    ...
    this.mainWindow.once('ready-to-show', () => {
      logger.info('WindowManager: Window ready-to-show event fired');
      try {
        this.mainWindow?.show();
        logger.info('WindowManager: Window show() called successfully');
    ...
    this.mainWindow.on('show', () => {
      logger.info('WindowManager: Window show event fired');
  ...
  private logWindowBounds(context: string): void {
    try {
      ...
      logger.info(`WindowManager: Window size (${context})`, {
        bounds,
        contentBounds,
        isMaximized,
      });
  ...
  private attachWebContentsDebug(webContents: Electron.WebContents): void {
    ...
    webContents.on('did-finish-load', () =>
      logger.info('WindowManager WebContents: did-finish-load')
    );
    ...
    webContents.on('responsive', () => logger.info('WindowManager WebContents: responsive'));

    webContents.on('console-message', (_event, level, message, line, sourceId) =>
      logger.debug('WindowManager Console', { level, message, line, sourceId })
    );
  }
  ...
  private async loadWindowContent(): Promise<boolean> {
    ...
    logger.info('WindowManager: Checking content loading strategy', {
      isPackaged: app.isPackaged,
      distIndex,
  ...
  private async loadPackagedContent(distIndex: string): Promise<boolean> {
    logger.info('WindowManager: Loading packaged index file', {
      distIndex,
    ...
    try {
      ...
      logger.info('WindowManager: Packaged content loaded successfully');
  ...
  private async loadDevelopmentContent(distIndex: string): Promise<boolean> {
    logger.info('WindowManager: Development mode - checking dev server');
    ...
    logger.info('WindowManager: Dev server check timeout', { timeout, hasBuiltFiles });
  ...
  private async loadDevServer(
    ...
    try {
      logger.info('WindowManager: Loading dev server', { url: devUrl });
      await this.mainWindow!.loadURL(devUrl);
      logger.info('WindowManager: Dev server loaded successfully');
      return 'dev';
    } catch (error) {
      ...
      if (fs.existsSync(fallbackPath)) {
        ...
        try {
          ...
          logger.info('WindowManager: Fallback content loaded successfully');
          return 'fallback';
        } catch (fallbackError) {
  ...
  private async waitForDevServer(url: string, timeoutMs: number = 20000): Promise<boolean> {
    logger.info('WindowManager: Starting dev server readiness check', { url, timeoutMs });
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      try {
        ...
        if ((response as Response).ok || (response as Response).status === 200) {
          logger.info('WindowManager: Dev server is ready', {
            attempts,
  ...
  closeWindow(): void {
    if (this.mainWindow && !this.mainWindow.isDestroyed()) {
      try {
        logger.info('WindowManager: Closing main window');
  ...
  async showPasswordUnlockWindow(): Promise<boolean> {
    logger.info('WindowManager: Showing password unlock window');
    ...
    return new Promise<boolean>((resolve) => {
      ...
      this.passwordUnlockWindow.on('closed', () => {
        logger.info('WindowManager: Password unlock window closed');
      ...
      this.loadPasswordUnlockContent(this.passwordUnlockWindow)
        .then(() => {
          logger.info('WindowManager: Password unlock window content loaded');
        })
        .catch((error) => {
  ...
  notifyPasswordUnlocked(): void {
    logger.info('WindowManager: Password unlock successful');
    if (this.passwordUnlockResolve) {
  ...
  closePasswordUnlockWindow(): void {
    if (this.passwordUnlockWindow && !this.passwordUnlockWindow.isDestroyed()) {
      logger.info('WindowManager: Closing password unlock window');
      try {
   ...
   */
  destroy(): void {
    logger.info('WindowManager: Cleaning up window manager');
    this.closeWindow();
    this.closeMeetingReminderWindow();
    this.closePasswordUnlockWindow();
  }
