...
export class DatabaseManager implements IDatabaseManager {
  ...
  constructor(
    ...
    logger.info('DatabaseManager: Initialized with encryption=%s', this.encryptionEnabled);
  }
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Initialize database
      this.dbPath = path.join(dataDir, 'notely.db');
  ...
  private async applyEncryptionKey(): Promise<void> {
    ...
    if (!dbExists || dbSize === 0) {
      ...
      logger.info('DatabaseManager: Creating new encrypted database');
    ...
    if (isMarked) {
      ...
      logger.info('DatabaseManager: Opening encrypted database');
      this.db.pragma(`key = ${sqlCipherKey}`);
