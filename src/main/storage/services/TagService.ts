...
export class TagService implements ITagService {
  ...
  /**
   * Mark a tag as needing sync (Joplin-style sync queue)
   */
  private markTagDirty(tagId: string): void {
    if (this.syncItemsService) {
      this.syncItemsService.markDirty('tags', tagId);
    }
  }

  /**
   * Mark a note_tag as needing sync (Joplin-style sync queue)
   */
  private markNoteTagDirty(noteTagId: string): void {
    if (this.syncItemsService) {
      this.syncItemsService.markDirty('note_tags', noteTagId);
    }
  }
  ...
  async create(input: CreateTagInput, userId?: string): Promise<string> {
    ...
    // Mark tag as needing sync
    this.markTagDirty(tagId);
  ...
  async update(input: UpdateTagInput): Promise<void> {
    ...
    this.markTagDirty(input.id);
  }

  async delete(tagId: string): Promise<void> {
    const now = Date.now();

    // Get note_tag IDs before deletion
    const noteTagIds = this.db
      .prepare('SELECT id FROM note_tags WHERE tag_id = ? AND deleted = 0')
      .all(tagId) as { id: string }[];
    ...
    // Mark all note_tags as needing sync
    for (const noteTag of noteTagIds) {
      this.markNoteTagDirty(noteTag.id);
    }
  }
  ...
  async addToNote(noteId: string, tagId: string): Promise<string> {
    ...
    // Mark note_tag as needing sync
    this.markNoteTagDirty(noteTagId);
  ...
  async setNoteTags(noteId: string, tagIds: string[]): Promise<void> {
    ...
    // Get existing note_tags for this note (for sync marking)
    const existingNoteTags = this.db
      .prepare('SELECT id FROM note_tags WHERE note_id = ?')
    ...
    // Mark all existing note_tags as needing sync (includes deletions and restores)
    for (const noteTag of existingNoteTags) {
      this.markNoteTagDirty(noteTag.id);
    }

    // Get newly created/restored note_tags and mark them dirty
