...
export class SummaryService implements ISummaryService {
  ...
  /**
   * Mark a summary as needing sync (Joplin-style sync queue)
   */
  private markDirty(summaryId: string): void {
    if (this.syncItemsService) {
      this.syncItemsService.markDirty('summaries', summaryId);
    }
  }
  ...
  async delete(summaryId: string): Promise<void> {
    ...
    if (didChange) {
      // Mark summary as needing sync (deletion will be synced)
