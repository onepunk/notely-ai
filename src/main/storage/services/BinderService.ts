...
export class BinderService implements IBinderService {
  ...
  /**
   * Mark a binder as needing sync (Joplin-style sync queue)
   * Called after create/update/delete operations
   */
  private markDirty(binderId: string): void {
  ...
  async create(name: string, userId?: string): Promise<string> {
    ...
    // Mark binder as needing sync
    this.markDirty(binderId);
  ...
  async delete(id: string): Promise<void> {
    ...
    // Get note IDs before deletion (for sync marking)
    ...
    // Mark all notes as needing sync
    if (this.syncItemsService) {
      for (const note of noteIds) {
        this.syncItemsService.markDirty('notes', note.id);
      }
    }
  }
  ...
  async reorder(order: string[]): Promise<void> {
    ...
    // Mark only changed binders as needing sync
    if (this.syncItemsService) {
      for (const binderId of changedBinderIds) {
        this.syncItemsService.markDirty('binders', binderId);
      }
    }
  }
