...
export class UserService implements IUserService {
   ...
   * Creates the system "Unassigned" binder with hardcoded UUID for sync consistency.
   */
  private createUserDefaultBindersSync(userId: string): void {
    ...
    for (const binder of DEFAULT_BINDERS) {
      ...
      // Track binders that were actually inserted (not ignored due to conflict)
      if (result.changes > 0) {
        createdBinderIds.push(binderId);
      }
    }

    // Mark newly created binders as needing sync
    if (this.syncItemsService && createdBinderIds.length > 0) {
      for (const binderId of createdBinderIds) {
        this.syncItemsService.markDirty('binders', binderId);
      }
      logger.debug('UserService: Marked default binders for sync', {
        count: createdBinderIds.length,
      });
    }
