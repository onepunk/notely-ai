...
export class NoteService implements INoteService {
  ...
  /**
   * Mark a note as needing sync (Joplin-style sync queue)
   * Called after create/update/delete operations
   */
  private markDirty(noteId: string): void {
    if (this.syncItemsService) {
      this.syncItemsService.markDirty('notes', noteId);
    }
  }
  ...
  async delete(noteId: string): Promise<void> {
    ...
    // Mark note as needing sync (deletion will be synced)
    this.markDirty(noteId);
  }
  ...
  async emptyTrash(): Promise<number> {
    const { ids, count } = await this.transactionManager.execute(() => {
      const rows = this.db.prepare('SELECT id FROM notes WHERE deleted=1').all() as Array<{
        id: string;
      }>;
      ...
      this.db
        .prepare(
          'DELETE FROM note_content_head WHERE note_id IN (SELECT id FROM notes WHERE deleted=1)'
        )
        .run();
      this.db
        .prepare(
          'DELETE FROM note_revisions WHERE note_id IN (SELECT id FROM notes WHERE deleted=1)'
        )
        .run();

      const deleteResult = this.db.prepare('DELETE FROM notes WHERE deleted=1').run();

      return { ids: rows.map((row) => row.id), count: deleteResult.changes ?? rows.length };
    });
  ...
  async createConflictCopy(
    ...
    // Mark conflict copy as needing sync
    this.markDirty(noteId);
  ...
  async resolveConflictUseConflictVersion(
    ...
    // Mark both notes as needing sync
    this.markDirty(canonicalNoteId);
    this.markDirty(conflictNoteId);
  }
  ...
  async resolveConflictKeepCanonical(conflictNoteId: string): Promise<void> {
    ...
    // Mark conflict copy as needing sync (deletion)
    this.markDirty(conflictNoteId);
  }
