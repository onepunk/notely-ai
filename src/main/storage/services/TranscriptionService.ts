...
export class TranscriptionService implements ITranscriptionService {
  ...
  /**
   * Mark a transcription session as needing sync (Joplin-style sync queue)
   */
  private markDirty(sessionId: string): void {
    if (this.syncItemsService) {
      this.syncItemsService.markDirty('transcriptions', sessionId);
    }
  }
  ...
  async completeSession(sessionId: string, endTime?: number): Promise<void> {
    ...
    // Mark transcription as needing sync
  ...
  async deleteSession(sessionId: string): Promise<void> {
    ...
    // Mark transcription as needing sync (deletion will be synced)
    this.markDirty(sessionId);
  }
