...
export type SyncConfigRow = {
  id: number; // Always 1
  desktop_user_uuid: string | null;
  sync_enabled: number;
  last_push_at: number | null;
  last_pull_at: number | null;
  created_at: number;
  updated_at: number;
  // Auth fields removed - see migration 021
  // Use AuthService for: server_url, server_user_id, access_token, refresh_token, token_expires_at
};

export type SyncLogRow = {
  id: number;
  operation: 'push' | 'pull' | 'link' | 'restore';
...
// Cursor-based Sync Tables (added in migration 029)
export type SyncItemRow = {
  entity_type: 'binders' | 'notes' | 'transcriptions' | 'summaries' | 'tags' | 'note_tags';
  entity_id: string;
  sync_time: number;
