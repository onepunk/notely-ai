...
export class StorageService implements IStorageService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('Ensuring local user exists...');
      ...
      logger.info('Local user ready', { userId: userId.substring(0, 8) + '...' });
    } catch (error) {
      logger.error(
  ...
  private async runEncryptionMigrationIfNeeded(): Promise<void> {
    ...
    const debugDbEnabled = process.env.DEBUG_DB === 'true' || process.env.DEBUG_DB === '1';
    if (debugDbEnabled) {
      logger.info('StorageService: Skipping encryption migration (DEBUG_DB mode)');
      return;
    }
    ...
    if (result.migrated) {
      logger.info('StorageService: Database encryption migration completed successfully', {
