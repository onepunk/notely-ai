...
export class MigrationRunner {
  ...
  async applyBaseline(baselineVersion = 9): Promise<BaselineResult> {
    ...
    try {
      logger.info('Applying baseline schema v%d for fresh installation', baselineVersion);
      ...
      // Apply baseline in a transaction
      this.db.transaction(() => {
        // Execute the baseline schema
        this.db.exec(baselineSQL);
      ...
      result.success = true;
      logger.info('Baseline schema v%d applied successfully', baselineVersion);
  ...
  async recoverPartialMigration(): Promise<MigrationResult[]> {
    ...
    try {
      const currentVersion = this.getCurrentVersion();
      logger.info('Current version in partial state: %d', currentVersion);
  ...
  private getBaselineSchemaPath(version: number): string {
    ...
    const versionedPath = path.join(__dirname, '..', `baseline-schema-v${version}.sql`);
    if (fs.existsSync(versionedPath)) {
      return versionedPath;
    }

    // Fall back to generic baseline file
    const genericPath = path.join(__dirname, '..', 'baseline-schema.sql');
