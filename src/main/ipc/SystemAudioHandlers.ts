...
export class SystemAudioHandlers {
   ...
   */
  register(): void {
    logger.info('SystemAudioHandlers: Registering IPC handlers');

    ipcMain.handle(IPC.SYSTEM_AUDIO_IS_SUPPORTED, this.handleIsSupported.bind(this));
    ipcMain.handle(IPC.SYSTEM_AUDIO_GET_INIT_ERROR, this.handleGetInitError.bind(this));

    logger.info('SystemAudioHandlers: All handlers registered successfully');
  }
   ...
   */
  cleanup(): void {
    logger.info('SystemAudioHandlers: Cleaning up IPC handlers');

    const handlers = [IPC.SYSTEM_AUDIO_IS_SUPPORTED, IPC.SYSTEM_AUDIO_GET_INIT_ERROR];
