...
export interface SettingsHandlersDependencies {
  ...
  /** Callback to notify SyncLifecycleManager when sync setting changes */
  onSyncSettingChanged?: () => void | Promise<void>;
}
...
export class SettingsHandlers {
   ...
   */
  register(): void {
    logger.info('SettingsHandlers: Registering IPC handlers');

    // Settings IPC handlers
    ipcMain.handle(IPC.SETTINGS_GET, this.handleGetSetting.bind(this));
    ipcMain.handle(IPC.SETTINGS_SET, this.handleSetSetting.bind(this));
    ipcMain.handle(IPC.SETTINGS_LIST_BY_PREFIX, this.handleListSettingsByPrefix.bind(this));
    ipcMain.on(IPC.SETTINGS_BROADCAST, this.handleBroadcastSetting.bind(this));

    // User Profile IPC handlers
    ipcMain.handle(IPC.USER_GET_PROFILE, this.handleGetUserProfile.bind(this));
    ipcMain.handle(IPC.USER_SAVE_PROFILE, this.handleSaveUserProfile.bind(this));

    logger.info('SettingsHandlers: All handlers registered successfully');
  }
  ...
  private async handleSetSetting(
    ...
    try {
      ...
      if (this.deps.mainWindow) {
        this.deps.mainWindow.webContents.send(IPC.EVT_SETTINGS_CHANGED, { key, value });
      }

      // Notify SyncLifecycleManager when sync setting changes
      if (key === 'syncEnabled' && this.deps.onSyncSettingChanged) {
        try {
          await this.deps.onSyncSettingChanged();
        } catch (error) {
          logger.warn('SettingsHandlers: Failed to notify sync setting change', {
            error: error instanceof Error ? error.message : error,
          });
        }
      }
  ...
  private handleBroadcastSetting(
    ...
    try {
      ...
      if (this.deps.mainWindow) {
        this.deps.mainWindow.webContents.send(IPC.EVT_SETTINGS_CHANGED, payload);
      }
  ...
  private async handleSaveUserProfile(
    ...
    try {
      ...
      if (this.deps.mainWindow) {
        this.deps.mainWindow.webContents.send(IPC.EVT_PROFILE_CHANGED);
      }
  ...
  async sendSettingsHydration(): Promise<void> {
    try {
      if (this.deps.mainWindow) {
        ...
        this.deps.mainWindow.webContents.send(IPC.EVT_SETTINGS_HYDRATE, settings);
   ...
   */
  cleanup(): void {
    logger.info('SettingsHandlers: Cleaning up IPC handlers');

    const handlers = [
      IPC.SETTINGS_GET,
      IPC.SETTINGS_SET,
      IPC.SETTINGS_LIST_BY_PREFIX,
      IPC.SETTINGS_BROADCAST,
      IPC.USER_GET_PROFILE,
      IPC.USER_SAVE_PROFILE,
    ];
    ...
    logger.info('SettingsHandlers: Cleanup completed');
  }
