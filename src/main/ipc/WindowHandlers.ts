...
export class WindowHandlers {
   ...
   */
  register(): void {
    logger.info('WindowHandlers: Registering IPC handlers');

    // Window control handlers
    ipcMain.on(IPC.WINDOW_CONTROL, this.handleWindowControl.bind(this));

    // Logging handlers
    ipcMain.on(IPC.LOG_SET_LEVEL, this.handleSetLogLevel.bind(this));
    ipcMain.on(IPC.LOG_INFO, this.handleLogInfo.bind(this));
    ipcMain.on(IPC.LOG_WARN, this.handleLogWarn.bind(this));
    ipcMain.on(IPC.LOG_ERROR, this.handleLogError.bind(this));
    ipcMain.on(IPC.LOG_DEBUG, this.handleLogDebug.bind(this));

    // Renderer lifecycle handlers
    ipcMain.on(IPC.RENDERER_READY, this.handleRendererReady.bind(this));

    // App info handlers
    ipcMain.handle(IPC.APP_GET_VERSION, this.handleGetVersion.bind(this));
    ipcMain.handle(IPC.APP_IS_DEVELOPMENT, this.handleIsDevelopment.bind(this));

    // Window overlay customization (Windows only)
    ipcMain.handle(IPC.WINDOW_SET_TITLEBAR_OVERLAY, this.handleSetTitlebarOverlay.bind(this));

    // External URL handler
    ipcMain.handle(IPC.WINDOW_OPEN_EXTERNAL, this.handleOpenExternal.bind(this));

    logger.info('WindowHandlers: All handlers registered successfully');
  }
   ...
   */
  cleanup(): void {
    logger.info('WindowHandlers: Cleaning up IPC handlers');
    ...
    const handlers = [
      ...
      IPC.RENDERER_READY,
      IPC.APP_GET_VERSION,
      IPC.APP_IS_DEVELOPMENT,
      IPC.WINDOW_SET_TITLEBAR_OVERLAY,
      IPC.WINDOW_OPEN_EXTERNAL,
    ];
    ...
    logger.info('WindowHandlers: Cleanup completed');
  }
}
