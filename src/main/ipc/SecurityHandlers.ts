...
export class SecurityHandlers {
   ...
   */
  register(): void {
    logger.info('SecurityHandlers: Registering IPC handlers');

    // Status
    ipcMain.handle(IPC.SECURITY_GET_PASSWORD_STATUS, this.handleGetStatus.bind(this));

    // Password operations
    ipcMain.handle(IPC.SECURITY_ENABLE_PASSWORD, this.handleEnablePassword.bind(this));
    ipcMain.handle(IPC.SECURITY_DISABLE_PASSWORD, this.handleDisablePassword.bind(this));
    ipcMain.handle(IPC.SECURITY_VERIFY_PASSWORD, this.handleVerifyPassword.bind(this));
    ipcMain.handle(IPC.SECURITY_CHANGE_PASSWORD, this.handleChangePassword.bind(this));
    ipcMain.handle(IPC.SECURITY_LOCK, this.handleLock.bind(this));
    ipcMain.handle(IPC.SECURITY_CLEAR_REMEMBER, this.handleClearRemember.bind(this));

    // Recovery key operations
    ipcMain.handle(IPC.SECURITY_EXPORT_RECOVERY_KEY, this.handleExportRecoveryKey.bind(this));
    ipcMain.handle(IPC.SECURITY_IMPORT_RECOVERY_KEY, this.handleImportRecoveryKey.bind(this));
    ipcMain.handle(
      IPC.SECURITY_MARK_RECOVERY_KEY_SHOWN,
      this.handleMarkRecoveryKeyShown.bind(this)
    );
    ipcMain.handle(IPC.SECURITY_RESET_WITH_RECOVERY, this.handleResetPassword.bind(this));

    logger.info('SecurityHandlers: All handlers registered successfully');
  }
  ...
  private broadcastStatusChange(): void {
    if (this.deps.mainWindow && !this.deps.mainWindow.isDestroyed()) {
      this.getService()
        .getStatus()
        .then((status) => {
          this.deps.mainWindow?.webContents.send(IPC.EVT_SECURITY_STATUS_CHANGED, status);
        })
        .catch((error) => {
   ...
   */
  cleanup(): void {
    logger.info('SecurityHandlers: Cleaning up IPC handlers');

    const handlers = [
      IPC.SECURITY_GET_PASSWORD_STATUS,
      IPC.SECURITY_ENABLE_PASSWORD,
      IPC.SECURITY_DISABLE_PASSWORD,
      IPC.SECURITY_VERIFY_PASSWORD,
      IPC.SECURITY_CHANGE_PASSWORD,
      IPC.SECURITY_LOCK,
      IPC.SECURITY_CLEAR_REMEMBER,
      IPC.SECURITY_EXPORT_RECOVERY_KEY,
      IPC.SECURITY_IMPORT_RECOVERY_KEY,
      IPC.SECURITY_MARK_RECOVERY_KEY_SHOWN,
      IPC.SECURITY_RESET_WITH_RECOVERY,
    ];
    ...
    logger.info('SecurityHandlers: Cleanup completed');
  }
}
