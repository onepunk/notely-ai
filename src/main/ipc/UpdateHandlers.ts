...
export class UpdateHandlers {
   ...
   */
  register(): void {
    logger.info('UpdateHandlers: Registering IPC handlers');

    // Register handlers
    ipcMain.handle(IPC.UPDATE_CHECK, this.handleCheckForUpdate.bind(this));
    ipcMain.handle(IPC.UPDATE_GET_CACHED, this.handleGetCached.bind(this));
    ipcMain.handle(IPC.UPDATE_OPEN_DOWNLOAD, this.handleOpenDownload.bind(this));
    ipcMain.handle(IPC.UPDATE_DISMISS, this.handleDismiss.bind(this));
    ipcMain.handle(IPC.UPDATE_IS_DISMISSED, this.handleIsDismissed.bind(this));
    ipcMain.handle(IPC.UPDATE_GET_VERSION, this.handleGetVersion.bind(this));

    // New download handlers
    ipcMain.handle(IPC.UPDATE_START_DOWNLOAD, this.handleStartDownload.bind(this));
    ipcMain.handle(IPC.UPDATE_GET_DOWNLOAD_STATUS, this.handleGetDownloadStatus.bind(this));
    ipcMain.handle(IPC.UPDATE_IS_DOWNLOAD_READY, this.handleIsDownloadReady.bind(this));
    ipcMain.handle(IPC.UPDATE_INSTALL_AND_RESTART, this.handleInstallAndRestart.bind(this));
    ipcMain.handle(IPC.UPDATE_CANCEL_DOWNLOAD, this.handleCancelDownload.bind(this));
    ipcMain.handle(IPC.UPDATE_RESET_DOWNLOAD, this.handleResetDownload.bind(this));

    // Subscribe to update service events
    this.setupEventListeners();

    logger.info('UpdateHandlers: IPC handlers registered');
  }
  ...
  cleanup(): void {
    ...
    ipcMain.removeHandler(IPC.UPDATE_GET_CACHED);
    ipcMain.removeHandler(IPC.UPDATE_OPEN_DOWNLOAD);
    ipcMain.removeHandler(IPC.UPDATE_DISMISS);
    ipcMain.removeHandler(IPC.UPDATE_IS_DISMISSED);
    ipcMain.removeHandler(IPC.UPDATE_GET_VERSION);
    ipcMain.removeHandler(IPC.UPDATE_START_DOWNLOAD);
    ipcMain.removeHandler(IPC.UPDATE_GET_DOWNLOAD_STATUS);
    ipcMain.removeHandler(IPC.UPDATE_IS_DOWNLOAD_READY);
    ipcMain.removeHandler(IPC.UPDATE_INSTALL_AND_RESTART);
    ipcMain.removeHandler(IPC.UPDATE_CANCEL_DOWNLOAD);
    ipcMain.removeHandler(IPC.UPDATE_RESET_DOWNLOAD);
  ...
  private broadcastUpdateAvailable(info: UpdateInfo): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_AVAILABLE, info);
  ...
  private broadcastUpdateDismissed(version: string): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_DISMISSED, version);
  ...
  private broadcastDownloadStarted(): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_DOWNLOAD_STARTED);
  ...
  private broadcastDownloadProgress(progress: number): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_DOWNLOAD_PROGRESS, progress);
    } catch (error) {
  ...
  private broadcastDownloadComplete(downloadPath: string): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_DOWNLOAD_COMPLETE, downloadPath);
  ...
  private broadcastDownloadError(error: string): void {
    ...
    try {
      this.mainWindow.webContents.send(IPC.EVT_UPDATE_DOWNLOAD_ERROR, error);
