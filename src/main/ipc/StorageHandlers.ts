...
export class StorageHandlers {
   ...
   */
  register(): void {
    logger.info('StorageHandlers: Registering IPC handlers');

    // Notes IPC handlers
    ipcMain.handle(IPC.STORAGE_CREATE_NOTE, this.handleCreateNote.bind(this));
    ipcMain.handle(IPC.STORAGE_SAVE_NOTE, this.handleSaveNote.bind(this));
    ipcMain.handle(IPC.STORAGE_GET_NOTE, this.handleGetNote.bind(this));
    ipcMain.handle(IPC.STORAGE_LIST_NOTES_BY_BINDER, this.handleListNotesByBinder.bind(this));
    ipcMain.handle(IPC.STORAGE_LIST_UNASSIGNED_NOTES, this.handleListUnassignedNotes.bind(this));
    ipcMain.handle(IPC.STORAGE_LIST_ALL_NOTES, this.handleListAllNotes.bind(this));
    ipcMain.handle(
      IPC.STORAGE_LIST_NOTES_BY_CREATED,
      this.handleListNotesByCreatedBetween.bind(this)
    );
    ipcMain.handle(IPC.STORAGE_LIST_DELETED_NOTES, this.handleListDeletedNotes.bind(this));
    ipcMain.handle(IPC.STORAGE_EMPTY_TRASH, this.handleEmptyTrash.bind(this));
    ipcMain.handle(IPC.STORAGE_DELETE_NOTE, this.handleDeleteNote.bind(this));
    ipcMain.handle(IPC.STORAGE_MOVE_NOTE, this.handleMoveNote.bind(this));
    ipcMain.handle(IPC.STORAGE_SET_STARRED, this.handleSetStarred.bind(this));
    ipcMain.handle(IPC.STORAGE_LIST_STARRED_NOTES, this.handleListStarredNotes.bind(this));
    ipcMain.handle(IPC.STORAGE_SET_ARCHIVED, this.handleSetArchived.bind(this));
    ipcMain.handle(IPC.STORAGE_LIST_ARCHIVED_NOTES, this.handleListArchivedNotes.bind(this));
    ipcMain.handle(IPC.STORAGE_SEARCH, this.handleSearch.bind(this));

    // Binders IPC handlers
    ipcMain.handle(IPC.STORAGE_LIST_BINDERS, this.handleListBinders.bind(this));
    ipcMain.handle(IPC.STORAGE_GET_DEFAULT_BINDER_ID, this.handleGetDefaultBinderId.bind(this));
    ipcMain.handle(IPC.STORAGE_CREATE_BINDER, this.handleCreateBinder.bind(this));
    ipcMain.handle(IPC.STORAGE_RENAME_BINDER, this.handleRenameBinder.bind(this));
    ipcMain.handle(IPC.STORAGE_UPDATE_BINDER, this.handleUpdateBinder.bind(this));
    ipcMain.handle(IPC.STORAGE_DELETE_BINDER, this.handleDeleteBinder.bind(this));
    ipcMain.handle(IPC.STORAGE_REORDER_BINDERS, this.handleReorderBinders.bind(this));

    // Conflict IPC handlers (Phase 5)
    ipcMain.handle(IPC.STORAGE_LIST_CONFLICTS, this.handleListConflicts.bind(this));
    ipcMain.handle(IPC.STORAGE_COUNT_CONFLICTS, this.handleCountConflicts.bind(this));
    ipcMain.handle(IPC.STORAGE_GET_CONFLICTS_FOR_NOTE, this.handleGetConflictsForNote.bind(this));
    ipcMain.handle(
      IPC.STORAGE_GET_NOTES_WITH_CONFLICTS,
      this.handleGetNotesWithConflicts.bind(this)
    );
    ipcMain.handle(
      IPC.STORAGE_GET_NOTE_WITH_CONFLICT_META,
      this.handleGetNoteWithConflictMeta.bind(this)
    );
    ipcMain.handle(
      IPC.STORAGE_RESOLVE_USE_CONFLICT,
      this.handleResolveConflictUseConflictVersion.bind(this)
    );
    ipcMain.handle(
      IPC.STORAGE_RESOLVE_KEEP_CANONICAL,
      this.handleResolveConflictKeepCanonical.bind(this)
    );
    ipcMain.handle(IPC.STORAGE_GET_CONFLICTS_BINDER, this.handleGetConflictsBinder.bind(this));
    ipcMain.handle(
      IPC.STORAGE_HAS_UNRESOLVED_CONFLICTS,
      this.handleHasUnresolvedConflicts.bind(this)
    );
    ipcMain.handle(
      IPC.STORAGE_LIST_BINDERS_WITH_CONFLICTS,
      this.handleListBindersWithConflicts.bind(this)
    );

    logger.info('StorageHandlers: All handlers registered successfully');
  }
  ...
  private async handleSaveNote(_event: Electron.IpcMainInvokeEvent, input: unknown): Promise<void> {
    try {
      ...
      logger.debug('StorageHandlers: Note saved successfully', { noteId });
    } catch (error) {
  ...
  private async handleGetDefaultBinderId(
    ...
    try {
      logger.debug('StorageHandlers: Getting default binder ID');
  ...
  private async handleCreateBinder(
    ...
    try {
      ...
      logger.debug('StorageHandlers: Creating binder', { user_profile_id });
      ...
      logger.debug('StorageHandlers: Binder created successfully', { binderId });
      return binderId;
    } catch (error) {
  ...
  private async handleRenameBinder(
    ...
    try {
      ...
      logger.debug('StorageHandlers: Renaming binder', { id });
      ...
      logger.debug('StorageHandlers: Binder renamed successfully', { id });
    } catch (error) {
   ...
   */
  cleanup(): void {
    logger.info('StorageHandlers: Cleaning up IPC handlers');

    const handlers = [
      IPC.STORAGE_CREATE_NOTE,
      IPC.STORAGE_SAVE_NOTE,
      IPC.STORAGE_GET_NOTE,
      IPC.STORAGE_LIST_NOTES_BY_BINDER,
      IPC.STORAGE_LIST_UNASSIGNED_NOTES,
      IPC.STORAGE_LIST_ALL_NOTES,
      IPC.STORAGE_LIST_NOTES_BY_CREATED,
      IPC.STORAGE_LIST_DELETED_NOTES,
      IPC.STORAGE_EMPTY_TRASH,
      IPC.STORAGE_DELETE_NOTE,
      IPC.STORAGE_MOVE_NOTE,
      IPC.STORAGE_SET_STARRED,
      IPC.STORAGE_LIST_STARRED_NOTES,
      IPC.STORAGE_SET_ARCHIVED,
      IPC.STORAGE_LIST_ARCHIVED_NOTES,
      IPC.STORAGE_SEARCH,
      IPC.STORAGE_LIST_BINDERS,
      IPC.STORAGE_GET_DEFAULT_BINDER_ID,
      IPC.STORAGE_CREATE_BINDER,
      IPC.STORAGE_RENAME_BINDER,
      IPC.STORAGE_UPDATE_BINDER,
      IPC.STORAGE_DELETE_BINDER,
      IPC.STORAGE_REORDER_BINDERS,
      // Conflict handlers (Phase 5)
      IPC.STORAGE_LIST_CONFLICTS,
      IPC.STORAGE_COUNT_CONFLICTS,
      IPC.STORAGE_GET_CONFLICTS_FOR_NOTE,
      IPC.STORAGE_GET_NOTES_WITH_CONFLICTS,
      IPC.STORAGE_GET_NOTE_WITH_CONFLICT_META,
      IPC.STORAGE_RESOLVE_USE_CONFLICT,
      IPC.STORAGE_RESOLVE_KEEP_CANONICAL,
      IPC.STORAGE_GET_CONFLICTS_BINDER,
      IPC.STORAGE_HAS_UNRESOLVED_CONFLICTS,
      IPC.STORAGE_LIST_BINDERS_WITH_CONFLICTS,
    ];
    ...
    logger.info('StorageHandlers: Cleanup completed');
  }
